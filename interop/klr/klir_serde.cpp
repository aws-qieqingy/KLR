...
u8 t, c, l;
        ...
        if (t != 110) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
return Memory::hbm;
break;}
case 1: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
...
return Memory::reg;
break;}
default: throw std::runtime_error("Invalid value tag");
}
}
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 111) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 116) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 120) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 122) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 124) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 130) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 131) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 132) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
...
Ptr<Scale> Scale_des(FILE *in) {
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 175) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<ScaleImmWrapper> x = ptr<ScaleImmWrapper>();
x->i = Immediate_des(in);
return x;
break;}
case 1: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<ScaleTensorWrapper> x = ptr<ScaleTensorWrapper>();
...
Ptr<Axis> Axis_des(FILE *in) {
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 178) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<AxisAxWrapper> x = ptr<AxisAxWrapper>();
x->ax = Int_des(in);
return x;
break;}
case 1: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<AxisAxsWrapper> x = ptr<AxisAxsWrapper>();
x->axs = List_Int_des(in);
return x;
break;}
default: throw std::runtime_error("Invalid value tag");
}
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 180) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 134) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
return AluOp::abs;
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 135) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 136) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 137) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 138) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 139) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 140) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 141) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 142) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 143) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 144) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 0) throw std::runtime_error("Wrong number of elements");
...
x->op0 = AluOp_des(in);
x->operand1 = Option_Operand_des(in);
...
Ptr<NKIOperator> NKIOperator_des(FILE *in) {
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 174) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorActivateWrapper> x = ptr<NKIOperatorActivateWrapper>();
x->op = Activate_des(in);
return x;
break;}
case 1: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorAffineSelectWrapper> x = ptr<NKIOperatorAffineSelectWrapper>();
x->op = AffineSelect_des(in);
return x;
break;}
case 2: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorBatchNormAggregateWrapper> x = ptr<NKIOperatorBatchNormAggregateWrapper>();
x->op = BatchNormAggregate_des(in);
return x;
break;}
case 3: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorBatchNormStatsWrapper> x = ptr<NKIOperatorBatchNormStatsWrapper>();
x->op = BatchNormStats_des(in);
return x;
break;}
case 4: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorCopyWrapper> x = ptr<NKIOperatorCopyWrapper>();
x->op = Copy_des(in);
return x;
break;}
case 5: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorCopyPredicatedWrapper> x = ptr<NKIOperatorCopyPredicatedWrapper>();
x->op = CopyPredicated_des(in);
return x;
break;}
case 6: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorDmaCopyWrapper> x = ptr<NKIOperatorDmaCopyWrapper>();
x->op = DmaCopy_des(in);
return x;
break;}
case 7: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorDmaTransposeWrapper> x = ptr<NKIOperatorDmaTransposeWrapper>();
x->op = DmaTranspose_des(in);
return x;
break;}
case 8: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorDropoutWrapper> x = ptr<NKIOperatorDropoutWrapper>();
x->op = Dropout_des(in);
return x;
break;}
case 9: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorFindIndex8Wrapper> x = ptr<NKIOperatorFindIndex8Wrapper>();
x->op = FindIndex8_des(in);
return x;
break;}
case 10: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorIotaWrapper> x = ptr<NKIOperatorIotaWrapper>();
x->op = Iota_des(in);
return x;
break;}
case 11: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorLoadMaskRegisterWrapper> x = ptr<NKIOperatorLoadMaskRegisterWrapper>();
x->op = LoadMaskRegister_des(in);
return x;
break;}
case 12: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorLoadStationaryWrapper> x = ptr<NKIOperatorLoadStationaryWrapper>();
x->op = LoadStationary_des(in);
return x;
break;}
case 13: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorLocalGatherWrapper> x = ptr<NKIOperatorLocalGatherWrapper>();
x->op = LocalGather_des(in);
return x;
break;}
case 14: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorMatMulWrapper> x = ptr<NKIOperatorMatMulWrapper>();
x->op = MatMul_des(in);
return x;
break;}
case 15: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorMatchReplace8Wrapper> x = ptr<NKIOperatorMatchReplace8Wrapper>();
x->op = MatchReplace8_des(in);
return x;
break;}
case 16: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorMatchValueLoadWrapper> x = ptr<NKIOperatorMatchValueLoadWrapper>();
x->op = MatchValueLoad_des(in);
return x;
break;}
case 17: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorMax8Wrapper> x = ptr<NKIOperatorMax8Wrapper>();
x->op = Max8_des(in);
return x;
break;}
case 18: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorMemSetWrapper> x = ptr<NKIOperatorMemSetWrapper>();
x->op = MemSet_des(in);
return x;
break;}
case 19: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorRangeSelectWrapper> x = ptr<NKIOperatorRangeSelectWrapper>();
x->op = RangeSelect_des(in);
return x;
break;}
case 20: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorReciprocalWrapper> x = ptr<NKIOperatorReciprocalWrapper>();
x->op = Reciprocal_des(in);
return x;
break;}
case 21: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorScalarTensorTensorWrapper> x = ptr<NKIOperatorScalarTensorTensorWrapper>();
x->op = ScalarTensorTensor_des(in);
return x;
break;}
case 22: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorShuffleWrapper> x = ptr<NKIOperatorShuffleWrapper>();
x->op = Shuffle_des(in);
return x;
break;}
case 23: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorTensorReduceWrapper> x = ptr<NKIOperatorTensorReduceWrapper>();
x->op = TensorReduce_des(in);
return x;
break;}
case 24: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorTensorScalarWrapper> x = ptr<NKIOperatorTensorScalarWrapper>();
x->op = TensorScalar_des(in);
return x;
break;}
case 25: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorTensorScalarReduceWrapper> x = ptr<NKIOperatorTensorScalarReduceWrapper>();
x->op = TensorScalarReduce_des(in);
return x;
break;}
case 26: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorTensorTensorWrapper> x = ptr<NKIOperatorTensorTensorWrapper>();
x->op = TensorTensor_des(in);
return x;
break;}
case 27: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorTensorTensorScanWrapper> x = ptr<NKIOperatorTensorTensorScanWrapper>();
x->op = TensorTensorScan_des(in);
return x;
break;}
case 28: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorTransposeWrapper> x = ptr<NKIOperatorTransposeWrapper>();
x->op = Transpose_des(in);
return x;
break;}
case 29: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorNcMatMulWrapper> x = ptr<NKIOperatorNcMatMulWrapper>();
x->op = NcMatMul_des(in);
return x;
break;}
case 30: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorActivationReduceWrapper> x = ptr<NKIOperatorActivationReduceWrapper>();
x->op = ActivationReduce_des(in);
return x;
break;}
case 31: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
Ptr<NKIOperatorTensorPartitionReduceWrapper> x = ptr<NKIOperatorTensorPartitionReduceWrapper>();
...
Ptr<ActivationReduce> ActivationReduce_des(FILE *in) {
u8 t, c, l;
          if (!deserialize_tag(in, &t, &c, &l))
            throw std::runtime_error("Could not find tag");
          if (t != 176 || c != 0 || l != 8)
            throw std::runtime_error("Invalid Tag");
          Ptr<ActivationReduce> x = ptr<ActivationReduce>();
x->dst = TensorRef_des(in);
x->op = ActivationFunc_des(in);
x->data = TensorRef_des(in);
x->reduceOp = Option_AluOp_des(in);
x->reduceRes = Option_TensorRef_des(in);
x->bias = List_Immediate_des(in);
x->scale = Scale_des(in);
x->dtype = Option_Dtype_des(in);
return x;
}
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 101) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 103) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
...
break;}
case 1: {
 if (l != 3) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 104) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");
...
break;}
case 1: {
 if (l != 2) throw std::runtime_error("Wrong number of elements");
...
u8 t, c, l;
        if (!deserialize_tag(in, &t, &c, &l)) throw std::runtime_error("Could not read tag");
        if (t != 236) throw std::runtime_error("Unexpected type tag");
switch (c) {
case 0: {
 if (l != 1) throw std::runtime_error("Wrong number of elements");

...
bool Bool_List_ser(FILE *out, struct Bool_List* x) {
  u64 count = 0;
  for (struct Bool_List* node = x; node; node = node->next) count++;
  if (!cbor_encode_array_start(out, count)) return false;
  for (struct Bool_List* node = x; node; node = node->next)
    if (!cbor_encode_bool(out, node->b)) return false;
return true;
}

bool Nat_List_ser(FILE *out, struct Nat_List* x) {
  u64 count = 0;
  for (struct Nat_List* node = x; node; node = node->next) count++;
  if (!cbor_encode_array_start(out, count)) return false;
...
bool Int_List_ser(FILE *out, struct Int_List* x) {
  u64 count = 0;
  for (struct Int_List* node = x; node; node = node->next) count++;
  if (!cbor_encode_array_start(out, count)) return false;
  for (struct Int_List* node = x; node; node = node->next)
    if (!cbor_encode_int(out, node->i)) return false;
return true;
}

bool Float_List_ser(FILE *out, struct Float_List* x) {
  u64 count = 0;
  for (struct Float_List* node = x; node; node = node->next) count++;
  if (!cbor_encode_array_start(out, count)) return false;
  for (struct Float_List* node = x; node; node = node->next)
    if (!cbor_encode_float(out, node->f)) return false;
return true;
}

bool String_List_ser(FILE *out, struct String_List* x) {
  u64 count = 0;
  for (struct String_List* node = x; node; node = node->next) count++;
  if (!cbor_encode_array_start(out, count)) return false;
  for (struct String_List* node = x; node; node = node->next)
    if (!String_ser(out, node->s)) return false;
return true;
}
...
if (!Nat_Option_ser(out, x->lineEnd)) return false;
if (!Nat_Option_ser(out, x->columnEnd)) return false;
return true;
}
...
u64 count = 0;
    ...
    for (; count > 0; count--) {
      ...
      if (!Bool_des(in, region, &node->b)) return false;
    }
return true;
}
...
u64 count = 0;
    ...
    for (; count > 0; count--) {
      ...
      if (!Int_des(in, region, &node->i)) return false;
    }
return true;
}
...
u64 count = 0;
    ...
    for (; count > 0; count--) {
      ...
      if (!Float_des(in, region, &node->f)) return false;
    }
return true;
}
...
u64 count = 0;
    ...
    for (; count > 0; count--) {
      ...
      if (!String_des(in, region, &node->s)) return false;
    }
return true;
}
